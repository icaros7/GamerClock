// VCEAMF.cpp : Defines the initialization routines for the DLL.
//
// created by Unwinder
/////////////////////////////////////////////////////////////////////////////
#include "stdafx.h"
...
#endif
/////////////////////////////////////////////////////////////////////////////
static AFX_EXTENSION_MODULE VCEAMFDLL = { NULL, NULL };
/////////////////////////////////////////////////////////////////////////////
HINSTANCE					g_hModule				= 0;
CVCEAMFEncoder*				g_lpEncoder				= NULL;
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		if (!AfxInitExtensionModule(VCEAMFDLL, hInstance))
			return 0;

		new CDynLinkLibrary(VCEAMFDLL);

		g_hModule = hInstance;
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		AfxTermExtensionModule(VCEAMFDLL);
	}
	return 1;
}
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT Init(LPCSTR lpCfgPath)
{
	if (lpCfgPath)
	...
	g_lpEncoder = new CVCEAMFEncoder;

	return S_OK;
}
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT Uninit()
{
	if (g_lpEncoder)
	{
		delete g_lpEncoder;
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API DWORD GetEncodersNum()
{
	return 1;
}
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT GetEncoderCaps(DWORD dwEncoder, LPENCODER_CAPS lpCaps)
{
	if (!lpCaps)
		return E_POINTER;
	...
	case 0:
		...
		strcpy_s(lpCaps->szDesc, sizeof(lpCaps->szDesc), "AMD AMF VCE H.264");
		return S_OK;
	}
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT GetEncoderStat(DWORD dwEncoder, LPENCODER_STAT lpStat)
{
	if (!g_lpEncoder)
		return E_UNEXPECTED;
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT Configure(DWORD dwEncoder, HWND hParent)
{
	switch (dwEncoder)
	{
	case 0:
		CVCEAMFConfigurationDlg dlg;
		dlg.DoModal();
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT StartEncoding(DWORD dwEncoder, LPENCODER_INPUT lpInput)
{
	if (!g_lpEncoder)
		return E_UNEXPECTED;
	...
	case 0:
		...
		int memoryTypeIn	= GetConfigInt("AVC", "MemoryType"		, amf::AMF_MEMORY_DX9				);

		char szCfgPath[MAX_PATH];
		sprintf_s(szCfgPath, sizeof(szCfgPath), "%s\\VCEAMFPreset%d.cfg", g_szDllPath, preset);

		g_lastError			= g_lpEncoder->StartEncoding(lpInput->dwInputWidth, lpInput->dwInputHeight, lpInput->dwFramerate, targetBitrate, device, (amf::AMF_MEMORY_TYPE)memoryTypeIn, szCfgPath);

		if (g_lastError != VCEAMF_ERR_NONE)
			return E_FAIL;
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT Encode(DWORD dwEncoder, LPENCODER_INPUT lpInput)
{
	if (!g_lpEncoder)
		return E_UNEXPECTED;
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API BOOL StopEncoding(DWORD dwEncoder)
{
	if (!g_lpEncoder)
		return E_UNEXPECTED;
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API LPBYTE GetHeader(DWORD dwEncoder)
{
	if (!g_lpEncoder)
		return NULL;
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API DWORD GetHeaderSize(DWORD dwEncoder)
{
	if (!g_lpEncoder)
		return 0;
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT GetFrame(DWORD dwEncoder, LPENCODER_OUTPUT lpOutput)
{
	if (!g_lpEncoder)
		return E_UNEXPECTED;
	...
	{
	case 0:
		LPVCEAMF_ENCODED_FRAME lpFrame = g_lpEncoder->GetFrame();

		if (lpFrame)
		{
			lpOutput->dwFlags		= 0;

			if (lpFrame->dwFrameType & VCEAMF_FRAMETYPE_I)
...
//////////////////////////////////////////////////////////////////////
VCEAMF_API HRESULT ReleaseFrame(DWORD dwEncoder, LPENCODER_OUTPUT lpOutput)
{
	if (!g_lpEncoder)
		return E_UNEXPECTED;
	...
	{
	case 0:
		g_lpEncoder->DestroyFrame((LPVCEAMF_ENCODED_FRAME)lpOutput->lpContext);
